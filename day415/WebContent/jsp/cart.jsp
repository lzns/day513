...
<html>
		...
		<div class="container">
			...
			<c:if test="${not empty cart.map}">
						...
						<table class="table table-bordered">
							...
							<c:forEach items="${cart.items}" var ="p">
									<tr class="active">
										...
										<td width="10%">
											<input type="text" name="quantity" value="${p.count}" maxlength="4" size="10" readonly="readonly">
										</td>
										<td width="15%">
											<span class="subtotal">￥${p.subtotal }</span>
										</td>
										<td>
											<a href="javascript:void(0);" class="delete" onclick="deleteBypid('${p.product.pid}')">删除</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
				...
				<div style="margin-right:130px;">
					...
					登录后确认是否享有优惠&nbsp;&nbsp;
				</em> 赠送积分: <em style="color:#ff6600;">596</em>&nbsp; 商品金额: <strong style="color:#ff6600;">￥${cart.totalMoney}元</strong>
					</div>
	...
	<script type="text/javascript">
		function deleteBypid(pid){
			if(confirm("你确定要删除吗")){
				location.href=("${pageContext.request.contextPath}/cart?method=deleteByPid&pid="+pid+"");
			}
		}
		function clearCart(){
			if(confirm("你确要狠心清空购物车吗")){
